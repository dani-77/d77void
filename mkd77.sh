...
build_variant() {
	...
	case $variant in
		...
		mango-noctalia)
			COMMON=yes
			PKGS="$PKGS $XORG_PKGS $WAYLAND_PKGS $D77_CORE $CALAMARES cliphist ImageMagick kitty mangowc mate-polkit matugen noctalia-shell qt5-wayland qt6-wayland quickshell swaybg wayland-devel wayland-protocols xdg-desktop-portal-wlr"
			SERVICES="$SERVICES dbus elogind sddm NetworkManager polkitd power-profiles-daemon"
		;;
		niri)
			COMMON=yes
