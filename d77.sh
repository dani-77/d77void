...
include_installer() {
    if [ -x installer.sh ]; then
        ...
        rm "$installer"
	cp -r skel "$INCLUDEDIR"/etc
	cp slim.conf "$INCLUDEDIR"/etc
	cp wswap-X "$INCLUDEDIR"/usr/bin/
	cp rofi-power-menu "$INCLUDEDIR"/usr/bin/
	mkdir -p "$INCLUDEDIR"/usr/share/slim/themes
	cp -r greeny_dark "$INCLUDEDIR"/usr/share/slim/themes/
	mkdir -p "$INCLUDEDIR"/usr/share/polkit-1/rules.d
	cp 50-udisks.rules "$INCLUDEDIR"/usr/share/polkit-1/rules.d/
	mkdir -p "$INCLUDEDIR"/usr/share/void-artwork
	cp splash.png "$INCLUDEDIR"/usr/share/void-artwork
    else
